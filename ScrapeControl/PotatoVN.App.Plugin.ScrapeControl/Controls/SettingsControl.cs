...
using PotatoVN.App.Plugin.ScrapeControl.Controls.Prefabs;
...
public sealed class SettingsControl : UserControl
{
    ...
    private static FrameworkElement Wrap(FrameworkElement content)
    {
        ...
        content.Padding = new Thickness(18, 12, 18, 12);
        return content;
    }
    ...
    private static UIElement ToggleSetting(string title, string description, object source, string path)
    {
        ...
        return new Setting { Title = title, Description = description, Content = toggle };
    }
    ...
    private UIElement BuildStatusPanel()
    {
        ...
        return new Panel { Content = Wrap(stack) };
    }
    ...
    private void StartScrape()
    {
        ...
        if (ScrapeProfile.IsEmpty(ScrapeProfile.Build(_data)) && !_data.Staff)
        {
