...
using Widgets = PotatoVN.App.Plugin.ScrapeControl.Controls.Widgets;
...
/// 插件设置界面。
///
/// 全部用代码构建，不加载任何 XAML —— 插件场景下的 XAML 资源定位不可靠，
/// 一旦失败会让宿主崩溃。
/// </summary>
public sealed class SettingsControl : UserControl
{
    ...
    private readonly TextBlock _diagText = new()
    {
    ...
    private Widgets.VStack BuildRoot()
    {
        Widgets.VStack root = new();
    ...
    /// <summary>
    /// 给内容套一层内边距。
    /// WinUI 的 StackPanel 没有 Padding 属性，这里用 Grid 包一层实现。
    /// </summary>
    ...
    {
        TextBlock tb = new() { Text = text, TextWrapping = TextWrapping.Wrap };
        if (TryStyle("DescriptionTextStyle") is { } style)
        {
            tb.Style = style;
        }
        else
    ...
    private static Style? TryStyle(string key) =>
        Application.Current.Resources.TryGetValue(key, out object? value) && value is Style s ? s : null;
    ...
    private static UIElement ToggleSetting(string title, string description, object source, string path)
    {
        ...
        return Card(new Widgets.Setting
        {
            TitleText = title,
    ...
    private static UIElement Card(UIElement inner) => new Widgets.Panel { Body = inner };
    ...
    public void RefreshStatus()
    {
        ...
        _diagText.Text = $"诊断：刮削服务={HostBridge.IsAvailable} / "
    ...
    private UIElement BuildPresetPanel()
    {
        ...
        stack.Children.Add(Description("预设会一次性改好下面的六个开关，选完仍可单独微调。"));
    ...
    {
        Button button = new() { Content = label, Padding = new Thickness(12, 5, 12, 5) };
        if (TryStyle("AccentButtonStyle") is { } style) button.Style = style;
    ...
    private UIElement BuildCategoryPanel()
    {
        ...
        stack.Children.Add(ToggleSetting("封面图", "游戏列表里显示的小图。",
            _data, nameof(PluginData.Image)));

        stack.Children.Add(ToggleSetting("标题图", "详情页顶部的横向大图，体积较大。",
    ...
    private UIElement BuildOptionPanel()
    {
        ...
        stack.Children.Add(Card(new Widgets.Setting
        {
            TitleText = "游戏之间间隔",
            DescriptionText = "批量刮削时每个游戏之间的等待时间，用于缓解信息源限流。被限流时可调大。",
            Body = delayCombo,
        }));
        ...
        stack.Children.Add(ToggleSetting("包含子游戏库", "连带处理当前库下的子库。",
    ...
    private UIElement BuildActionPanel()
    {
        ...
        if (TryStyle("AccentButtonStyle") is { } style) scrapeButton.Style = style;
    ...
    private void StartScrape()
    {
        ...
        _ = Plugin.HostApi!.AddBgTask(new ScrapeByCategoryTask(Plugin.HostApi!, _data, games));
    ...
    private void StartStaffFetch()
    {
        ...
        _ = Plugin.HostApi!.AddBgTask(new FetchStaffTask(Plugin.HostApi!, games));
    }
}
